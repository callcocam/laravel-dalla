                        ...
                        <div class="card mt-4 mb-4">
                            ...
                            <div class="card-footer d-sm-flex justify-content-sm-between align-items-sm-center">
                                @can('admin.events-last.edit')
                                    <a class="btn btn-primary btn-rounded" href="{{ route('admin.events-last.edit',$row->id) }}">{{ __('Editar Evento') }}</a>
                                @endcan
                                @can('admin.events-last.show')
                                    <a class="btn btn-warning btn-rounded" href="{{ route('admin.events-last.show',$row->id) }}">{{ __('Ver Evento') }}</a>
                                @endcan
                                @can('admin.events-last.destroy')
                                    <btn-delete-component event="{{ sprintf("form-%s", $row->id) }}">
                                        <form ref="form" action="{{ route('admin.events-last.destroy',$row->id) }}" method="POST">
                                            @csrf
                                            @method("DELETE")
                                        </form>
