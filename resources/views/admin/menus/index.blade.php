...
@section('breadcrumb')
    <div class="breadcrumb">
        ...
        <div style="right: 2%;position: absolute;">
            <a href="{{ route('admin.menus.create') }}" class="btn btn-success btn-rounded pull-right"><span class="icon i-Add-File"></span> {{ __('Create Menu') }}</a>
        </div>
    </div>
@endsection
@section('content')
                ...
                @foreach($rows as $row)
                    <div class="card m-2">
                        ...
                        <div class="card-body">
                                ...
                                <div class="card-header">
                                    <h6 class="card-title ul-collapse__icon--size ul-collapse__right-icon mb-0">
                                        <a  class="text-default collapsed" data-toggle="collapse"  href="#accordion-item-{{$row->id}}">
                                            <span><i class="i-Lock-User ul-accordion__font"> </i></span> {{ __("List of Sub Menus") }}
                                        </a>
                                    </h6>
                                </div>
                                @if($row->submenus)
                            ...
                            <hr>
                            {{ $row->description }}</p>
                            <a class="btn btn-primary btn-rounded" href="{{ route('admin.menus.edit',$row->id) }}">{{ __('Edit Menu') }}</a>
                            <a class="btn btn-primary btn-rounded" href="{{ route('admin.menus.show',$row->id) }}">{{ __('Delete Menu') }}</a>
